        ...
        await axiosClient.post("/order", formatData).then((response) => {
          ...
          if (response.error) {
            return notification["error"]({
              message: `Thông báo`,
              description: "Ngày bạn thuê đã có người khác đặt!",
            });
          }
    ...
    (async () => {
      try {
        ...
        const transformedData = cart.map(
          ({ id: product, quantity, promotion, rental_price, startDates, endDates }) => ({ product, quantity, promotion, rental_price, startDates, endDates })
        );
        let totalPrice = 0;
        ...
        for (let i = 0; i < transformedData.length; i++) {
          ...
          console.log(product);
          let price = product.rental_price * product.quantity;
          totalPrice += price;
        }
